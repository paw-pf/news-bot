...
func (s *SourcePostgresStorage) SourcesByID(ctx context.Context, id int64) (*model.Source, error) {
	conn, err := s.db.Connx(ctx)
	if err != nil {
		return nil, err
	}
	...
	var sources dbSource
	if err := conn.GetContext(ctx, &sources, "SELECT * FROM sources WHERE id =$1", id); err != nil {
		return nil, err
	}

	return (*model.Source)(&sources), nil
}
